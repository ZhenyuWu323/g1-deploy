...
class Controller:
    def __init__(self, config: Config) -> None:
        ...
        self.upper_body_default_pos = config.upper_body_default_pos.copy()
        ...
        self.frame_stack = deque(maxlen=config.history_length)
    ...
    def default_pos_state(self):
        ...
        for _ in range(5):
            self.frame_stack.append(self.obs.copy())
        # init obs buffer
    ...
    def get_obs(self):
        ...
        for i in range(len(self.config.whole_body_joint2motor_idx)):
            motor_idx = self.config.whole_body_joint2motor_idx[i]
            q_t[i] = self.low_state.motor_state[motor_idx].q
            dq_t[i] = self.low_state.motor_state[motor_idx].dq
        ...
        self.obs[9 : 9 + self.config.num_actions] = q_t
        self.obs[9 + self.config.num_actions : 9 + 2 * self.config.num_actions] = dq_t
        self.obs[9 + 2 * self.config.num_actions : 9 + 3 * self.config.num_actions] = self.config.num_actions

        self.frame_stack.append(self.obs.copy())
        stacked_obs = np.concatenate(self.frame_stack, axis=0)  
        ...
        obs_pos = np.asarray(stacked_obs).reshape(5, 96)[:, 9:9 + self.config.num_actions].reshape(-1)
        obs_vel = np.asarray(stacked_obs).reshape(5, 96)[:, 9 + self.config.num_actions : 9 + 2 * self.config.num_actions].reshape(-1)
        obs_action = np.asarray(stacked_obs).reshape(5, 96)[:, 9 + 2 * self.config.num_actions : 9 + 3 * self.config.num_actions].reshape(-1)
        ...
        big_group_major = np.clip(big_group_major, -self.config.clip_obervation, self.config.clip_obervation)
